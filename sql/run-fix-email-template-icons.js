// Fix corrupted email template icons in database
// Run: node sql/run-fix-email-template-icons.js
...
const templates = [
    {
        key: 'OE_FULL',
        subject: '📋 OE Inspection Report - {{storeName}} - {{documentNumber}} ({{totalScore}}%)',
        bodyReplacements: [
            { find: 'corrupted', replaceAll: true }
        ]
    },
    {
        key: 'OE_ACTION_PLAN',
        subject: '📝 OE Action Plan Required - {{storeName}} - {{totalFindings}} Findings ({{highFindings}} High)',
    },
    {
        key: 'OHS_FULL',
        subject: '🦺 OHS Inspection Report - {{storeName}} - {{documentNumber}} ({{totalScore}}%)',
    },
    {
        key: 'OHS_ACTION_PLAN',
        subject: '⚠️ OHS Action Plan Required - {{storeName}} - {{totalFindings}} Safety Findings',
    },
    {
        key: 'OE_VERIFICATION_SUBMITTED',
        subject: '[OE] Action Item Verification Submitted - {{storeName}} - {{documentNumber}}',
    },
    {
        key: 'THEFT_INCIDENT_REPORT',
        subject: '[Theft Incident] {{storeName}} - {{incidentDate}} - {{currency}} {{stolenValue}}',
    },
    {
    ...
    {
        key: 'FIVEDAYS_INITIATE',
        subject: '📅 5 Days Cycle Started - Begin Recording Expired Items',
    },
    {
        key: 'FIVEDAYS_DAILY',
        subject: '📋 Day {{dayNumber}} Reminder - 5 Days Expired Items',
    },
    {
        key: 'FIVEDAYS_OVERDUE',
        subject: '🚨 WARNING: Missing 5 Days Data - Audit Impact',
    },
    {
        key: 'OE_INSPECTION_ESCALATION',
        subject: '🚨 [OE] ESCALATION: Action Plan Requires Attention - {{storeName}} - {{documentNumber}}',
    },
    {
        key: 'OHS_INSPECTION_ESCALATION',
        subject: '🚨 [OHS] SAFETY ESCALATION: Immediate Attention Required - {{storeName}} - {{documentNumber}}',
    },
];

// Full body templates
const bodies = {
    'OE_FULL': `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
...
<body>
    ...
    'OE_VERIFICATION_SUBMITTED': `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
...
<body>
    ...
    'THEFT_INCIDENT_REPORT': `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
...
<body>
    ...
    'BROADCAST_MESSAGE': `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    'FIVEDAYS_INITIATE': `<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
...
<body>
    ...
    'OE_INSPECTION_ESCALATION': `<div style="font-family: Segoe UI, Arial, sans-serif; max-width: 600px; margin: 20px auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
    ...
    'OHS_INSPECTION_ESCALATION': `<div style="font-family: Segoe UI, Arial, sans-serif; max-width: 600px; margin: 20px auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #c0392b 0%, #e74c3c 100%); color: white; padding: 30px; text-align: center;">
        ...
        for (const tmpl of templates) {
            ...
            if (body) {
                ...
                await pool.request()
                    .input('key', sql.NVarChar, tmpl.key)
                    .input('subject', sql.NVarChar, tmpl.subject)
                    .query('UPDATE EmailTemplates SET SubjectTemplate = @subject, UpdatedAt = GETDATE() WHERE TemplateKey = @key');
                console.log('Fixed subject:', tmpl.key);
            }
        }
