-- Add Receiving Audit Email Templates
-- Run on both OEApp_UAT and OEApp_Live

-- RCV Full Report Template
IF NOT EXISTS (SELECT 1 FROM EmailTemplates WHERE TemplateKey = 'RCV_FULL')
BEGIN
    INSERT INTO EmailTemplates (TemplateKey, TemplateName, Module, ReportType, SubjectTemplate, BodyTemplate, IsActive, CreatedAt)
    VALUES (
        'RCV_FULL',
        'Receiving Audit Full Report Email',
        'RCV',
        'full',
        N'📦 Receiving Audit Report - {{storeName}} - {{documentNumber}} ({{totalScore}}%)',
        N'<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    PRINT 'Inserted RCV_FULL template';
END
ELSE
    PRINT 'RCV_FULL template already exists';
GO

-- RCV Action Plan Template
IF NOT EXISTS (SELECT 1 FROM EmailTemplates WHERE TemplateKey = 'RCV_ACTION_PLAN')
BEGIN
    INSERT INTO EmailTemplates (TemplateKey, TemplateName, Module, ReportType, SubjectTemplate, BodyTemplate, IsActive, CreatedAt)
    VALUES (
        'RCV_ACTION_PLAN',
        'Receiving Audit Action Plan Email',
        'RCV',
        'action-plan',
        N'📦 Receiving Audit Action Plan - {{storeName}} - {{totalFindings}} Findings',
        N'<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    PRINT 'Inserted RCV_ACTION_PLAN template';
END
ELSE
    PRINT 'RCV_ACTION_PLAN template already exists';
GO

-- RCV Escalation Template (for overdue action plans)
IF NOT EXISTS (SELECT 1 FROM EmailTemplates WHERE TemplateKey = 'RCV_ESCALATION')
BEGIN
    INSERT INTO EmailTemplates (TemplateKey, TemplateName, Module, ReportType, SubjectTemplate, BodyTemplate, IsActive, CreatedAt)
    VALUES (
        'RCV_ESCALATION',
        'Receiving Audit Escalation Email',
        'RCV',
        'escalation',
        N'🚨 ESCALATION: Receiving Audit Action Plan Overdue - {{storeName}} - {{daysOverdue}} Days',
        N'<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    PRINT 'Inserted RCV_ESCALATION template';
END
ELSE
    PRINT 'RCV_ESCALATION template already exists';
GO

SELECT TemplateKey, TemplateName, Module, ReportType, IsActive 
FROM EmailTemplates 
WHERE Module = 'RCV' 
