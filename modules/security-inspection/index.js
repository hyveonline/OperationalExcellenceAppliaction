 ...
 * Security Department Inspection App
...
// Maintenance integration not included in security inspection
...
const oeAuditUploadDir = path.join(__dirname, '..', '..', 'uploads', 'security-inspection');
...
const oeAuditStorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
        cb(null, 'sec-' + uniqueSuffix + path.extname(file.originalname));
    }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        res.send(`
            ...
            <head>
                <meta charset="UTF-8">
                <title>Security Inspection - ${process.env.APP_NAME}</title>
                <style>
            ...
            <body>
                ...
                <div class="container">
                    <div class="page-title">
                        <h2>Security Department Inspection</h2>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            <div class="stat-value" style="color: #dc2626;">${s.completed}</div>
                    ...
                    <div class="cards-grid">
                        <a href="/security-inspection/start" class="card">
                        ...
                        <a href="/security-inspection/list" class="card">
                            <div class="card-icon">📋</div>
                        ...
                        <a href="/security-inspection/settings" class="card">
                            <div class="card-icon">⚙️</div>
                            <div class="card-title">Settings</div>
                            <div class="card-desc">Configure inspection settings, document prefix, and thresholds.</div>
                        </a>
                        
                        <a href="/security-inspection/template-builder" class="card">
                            <div class="card-icon">🔧</div>
                        ...
                        <a href="/security-inspection/implementation-verification" class="card">
                            <div class="card-icon">✅</div>
                        ...
                        <a href="/security-inspection/cycle-dashboard" class="card">
                            <div class="card-icon">🔄</div>
                        ...
                        <a href="/security-inspection/escalation" class="card" style="border-left-color: #f59e0b;">
                            <div class="card-icon">🏢</div>
        ...
        console.error('Security Inspection dashboard error:', error);
        res.status(500).send(`
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT DISTINCT 
                    ...
                    (SELECT COUNT(*) FROM SEC_InspectionItems it 
                ...
                FROM SEC_Inspections i
                WHERE EXISTS (
                    SELECT 1 FROM SEC_InspectionItems it 
...
router.get('/api/department-reports/view/:reportId', async (req, res) => {
    try {
        ...
        const auditResult = await pool.request()
            ...
            .query(`SELECT * FROM SEC_Inspections WHERE Id = @reportId`);
        ...
        const itemsResult = await pool.request()
            ...
            .input('department', sql.NVarChar, department)
            .query(`
                SELECT * FROM SEC_InspectionItems 
    ...
    try {
        ...
        const result = await pool.request().query(`
            SELECT 
                ...
                (SELECT COUNT(*) FROM SEC_InspectionTemplateSections ts WHERE ts.TemplateId = t.Id AND ts.IsActive = 1) as sectionCount
            FROM SEC_InspectionTemplates t
...
router.post('/api/templates/schemas', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO SEC_InspectionTemplates (TemplateName, Description, CreatedBy, CreatedAt, IsActive)
...
router.get('/api/templates/schemas/:schemaId', async (req, res) => {
    try {
        ...
        const templateResult = await pool.request()
            ...
            .query(`SELECT Id as schemaId, TemplateName as schemaName, Description as description FROM SEC_InspectionTemplates WHERE Id = @id`);
        ...
        const sectionsResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionTemplateSections 
                WHERE TemplateId = @templateId AND IsActive = 1
                ORDER BY SectionOrder
            `);
        
        // Get categories and items for each section
        template.sections = [];
        for (const section of sectionsResult.recordset) {
            const categoriesResult = await pool.request()
                ...
                .query(`
                    SELECT Id as categoryId, CategoryName as categoryName, CategoryOrder as categoryOrder
                    FROM SEC_InspectionTemplateCategories
                ...
                const itemsResult = await pool.request()
                    ...
                    .query(`
                        SELECT Id as itemId, ReferenceValue as referenceValue, Question as title, Coefficient as coeff, 
                               ...
                               SubCategory as subCategory, IsQuantitative as isQuantitative, Range1From as range1From, Range1To as range1To,
            ...
            // Also get any legacy items directly on the section (no category)
            const legacyItemsResult = await pool.request()
                .input('sectionId', sql.Int, section.sectionId)
                .query(`
                    SELECT Id as itemId, ReferenceValue as referenceValue, Question as title, Coefficient as coeff, 
                           AnswerOptions as answer, Criteria as cr, Quantity as quantity, DefaultSeverity as defaultSeverity,
...
router.put('/api/templates/schemas/:schemaId', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('name', sql.NVarChar, schemaName)
            .input('desc', sql.NVarChar, description || '')
            .query(`UPDATE SEC_InspectionTemplates SET TemplateName = @name, Description = @desc WHERE Id = @id`);
        await pool.close();
...
router.delete('/api/templates/schemas/:schemaId', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.Int, req.params.schemaId)
            .query(`UPDATE SEC_InspectionTemplates SET IsActive = 0 WHERE Id = @id`);
        await pool.close();
        res.json({ success: true });
    } catch (error) {
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM SEC_InspectionTemplateCategories c WHERE c.SectionId = s.Id AND c.IsActive = 1) as categoryCount,
                    (SELECT COUNT(*) FROM SEC_InspectionTemplateItems i 
                     INNER JOIN SEC_InspectionTemplateCategories c ON i.CategoryId = c.Id 
                     WHERE c.SectionId = s.Id AND i.IsActive = 1) as itemCount
...
router.post('/api/templates/schemas/:schemaId/sections', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO SEC_InspectionTemplateSections (TemplateId, SectionName, SectionIcon, SectionOrder, IsActive)
...
router.put('/api/templates/sections/:sectionId', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('order', sql.Int, sectionNumber)
            .query(`
                UPDATE SEC_InspectionTemplateSections 
...
// Delete section (and its categories and items)
router.delete('/api/templates/sections/:sectionId', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        // Delete items that belong to categories in this section
        await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query(`DELETE i FROM SEC_InspectionTemplateItems i
                    INNER JOIN SEC_InspectionTemplateCategories c ON i.CategoryId = c.Id
                    WHERE c.SectionId = @sectionId`);
        // Also delete any items still linked directly to section (legacy)
        await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query('DELETE FROM SEC_InspectionTemplateItems WHERE SectionId = @sectionId');
        // Delete categories
        await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query('DELETE FROM SEC_InspectionTemplateCategories WHERE SectionId = @sectionId');
        // Delete section
        await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query('DELETE FROM SEC_InspectionTemplateSections WHERE Id = @sectionId');
        await pool.close();
        res.json({ success: true });
    } catch (error) {
...
// ==========================================
// Category CRUD (Department → Category level)
// ==========================================

// Get categories for a department/section
router.get('/api/templates/sections/:sectionId/categories', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        const result = await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query(`
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM SEC_InspectionTemplateItems i WHERE i.CategoryId = c.Id AND i.IsActive = 1) as itemCount
                FROM SEC_InspectionTemplateCategories c
                WHERE c.SectionId = @sectionId AND c.IsActive = 1
...
// Create category
router.post('/api/templates/sections/:sectionId/categories', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .input('order', sql.Int, categoryOrder || 0)
            .query(`
                INSERT INTO SEC_InspectionTemplateCategories (SectionId, CategoryName, CategoryOrder, IsActive)
                OUTPUT INSERTED.Id as categoryId
                VALUES (@sectionId, @name, @order, 1)
            `);
        await pool.close();
        res.json({ success: true, data: { categoryId: result.recordset[0].categoryId } });
    } catch (error) {
...
// Update category
router.put('/api/templates/categories/:categoryId', async (req, res) => {
    try {
        const { categoryName, categoryOrder } = req.body;
        const pool = await sql.connect(dbConfig);
        await pool.request()
            .input('id', sql.Int, req.params.categoryId)
            .input('name', sql.NVarChar, categoryName)
            .input('order', sql.Int, categoryOrder || 0)
            .query(`UPDATE SEC_InspectionTemplateCategories SET CategoryName = @name, CategoryOrder = @order WHERE Id = @id`);
        await pool.close();
        res.json({ success: true });
    } catch (error) {
...
router.delete('/api/templates/categories/:categoryId', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        await pool.request()
            .input('categoryId', sql.Int, req.params.categoryId)
            .query('DELETE FROM SEC_InspectionTemplateItems WHERE CategoryId = @categoryId');
        await pool.request()
            .input('categoryId', sql.Int, req.params.categoryId)
            .query('DELETE FROM SEC_InspectionTemplateCategories WHERE Id = @categoryId');
        await pool.close();
        res.json({ success: true });
    } catch (error) {
...
// ==========================================
// Item CRUD (under Categories)
// ==========================================

// Get items for a category
router.get('/api/templates/categories/:categoryId/items', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        const result = await pool.request()
            .input('categoryId', sql.Int, req.params.categoryId)
            .query(`
                SELECT Id as itemId, ReferenceValue as referenceValue, Question as title, Coefficient as coeff, Quantity as quantity, AnswerOptions as answer, Criteria as cr, DefaultSeverity as defaultSeverity,
                       SubCategory as subCategory, IsQuantitative as isQuantitative, Range1From as range1From, Range1To as range1To, Range2From as range2From, Range2To as range2To, Range3From as range3From
                FROM SEC_InspectionTemplateItems
                WHERE CategoryId = @categoryId AND IsActive = 1
                ORDER BY SubCategory, 
                    CAST(PARSENAME(REPLACE(ReferenceValue, '-', '.'), 2) AS INT),
...
// Legacy: Get items for a section (kept for backward compatibility)
router.get('/api/templates/sections/:sectionId/items', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT Id as itemId, ReferenceValue as referenceValue, Question as title, Coefficient as coeff, Quantity as quantity, AnswerOptions as answer, Criteria as cr, DefaultSeverity as defaultSeverity,
                       SubCategory as subCategory, CategoryId as categoryId, IsQuantitative as isQuantitative, Range1From as range1From, Range1To as range1To, Range2From as range2From, Range2To as range2To, Range3From as range3From
                FROM SEC_InspectionTemplateItems
                WHERE SectionId = @sectionId AND IsActive = 1
                ORDER BY 
                    CAST(PARSENAME(REPLACE(ReferenceValue, '-', '.'), 2) AS INT),
...
router.post('/api/templates/categories/:categoryId/items', async (req, res) => {
    try {
        const { referenceValue, title, coeff, quantity, answer, cr, defaultSeverity, subCategory, isQuantitative, range1From, range1To, range2From, range2To, range3From } = req.body;
        const pool = await sql.connect(dbConfig);
        // Get the SectionId from the category
        const catResult = await pool.request()
            .input('categoryId', sql.Int, req.params.categoryId)
            .query('SELECT SectionId FROM SEC_InspectionTemplateCategories WHERE Id = @categoryId');
        const sectionId = catResult.recordset[0]?.SectionId;
        const result = await pool.request()
            ...
            .input('quantity', sql.Int, quantity != null ? quantity : null)
            .input('answer', sql.NVarChar, answer || 'Yes,No,NA')
            ...
            .input('isQuantitative', sql.Bit, isQuantitative === true ? 1 : 0)
            .input('range1From', sql.Int, range1From != null ? range1From : null)
            .input('range1To', sql.Int, range1To != null ? range1To : null)
            .input('range2From', sql.Int, range2From != null ? range2From : null)
            .input('range2To', sql.Int, range2To != null ? range2To : null)
            .input('range3From', sql.Int, range3From != null ? range3From : null)
            .query(`
                INSERT INTO SEC_InspectionTemplateItems (SectionId, CategoryId, ReferenceValue, Question, Coefficient, Quantity, AnswerOptions, Criteria, DefaultSeverity, SubCategory, IsActive, IsQuantitative, Range1From, Range1To, Range2From, Range2To, Range3From)
                OUTPUT INSERTED.Id as itemId
                VALUES (@sectionId, @categoryId, @ref, @question, @coeff, @quantity, @answer, @criteria, @defaultSeverity, @subCategory, 1, @isQuantitative, @range1From, @range1To, @range2From, @range2To, @range3From)
            `);
        await pool.close();
...
// Legacy: Create item under section (kept for backward compatibility)
router.post('/api/templates/sections/:sectionId/items', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .input('range3From', sql.Int, range3From != null ? range3From : null)
            .query(`
                INSERT INTO SEC_InspectionTemplateItems (SectionId, ReferenceValue, Question, Coefficient, Quantity, AnswerOptions, Criteria, DefaultSeverity, SubCategory, IsActive, IsQuantitative, Range1From, Range1To, Range2From, Range2To, Range3From)
                OUTPUT INSERTED.Id as itemId
                VALUES (@sectionId, @ref, @question, @coeff, @quantity, @answer, @criteria, @defaultSeverity, @subCategory, 1, @isQuantitative, @range1From, @range1To, @range2From, @range2To, @range3From)
            `);
        await pool.close();
        res.json({ success: true, data: { itemId: result.recordset[0].itemId } });
    } catch (error) {
...
// Bulk create items under a category
router.post('/api/templates/categories/:categoryId/items/bulk', async (req, res) => {
    try {
        ...
        const existingResult = await pool.request()
            .input('categoryId', sql.Int, categoryId)
            .query(`SELECT ReferenceValue FROM SEC_InspectionTemplateItems WHERE CategoryId = @categoryId AND IsActive = 1`);
        ...
        for (const item of items) {
            ...
            await pool.request()
                ...
                .input('coeff', sql.Int, item.coeff || 1)
                .input('quantity', sql.Int, item.quantity || null)
                .input('answer', sql.NVarChar, item.answer || 'Yes,No,NA')
                .input('criteria', sql.NVarChar, item.cr || '')
                .input('defaultSeverity', sql.NVarChar, item.defaultSeverity || null)
                .input('subCategory', sql.NVarChar, item.subCategory || null)
                .query(`INSERT INTO SEC_InspectionTemplateItems (SectionId, CategoryId, ReferenceValue, Question, Coefficient, Quantity, AnswerOptions, Criteria, DefaultSeverity, SubCategory, IsActive) VALUES (@sectionId, @categoryId, @ref, @question, @coeff, @quantity, @answer, @criteria, @defaultSeverity, @subCategory, 1)`);
            created++;
...
// Legacy: Bulk create under section
router.post('/api/templates/sections/:sectionId/items/bulk', async (req, res) => {
    try {
        ...
        const existingResult = await pool.request()
            .input('sectionId', sql.Int, sectionId)
            .query(`SELECT ReferenceValue FROM SEC_InspectionTemplateItems WHERE SectionId = @sectionId AND IsActive = 1`);
        const existingRefs = new Set(existingResult.recordset.map(r => r.ReferenceValue?.toLowerCase()));
        ...
        for (const item of items) {
            ...
            await pool.request()
                ...
                .input('criteria', sql.NVarChar, item.cr || '')
                .input('defaultSeverity', sql.NVarChar, item.defaultSeverity || null)
                .query(`INSERT INTO SEC_InspectionTemplateItems (SectionId, ReferenceValue, Question, Coefficient, Quantity, AnswerOptions, Criteria, DefaultSeverity, IsActive) VALUES (@sectionId, @ref, @question, @coeff, @quantity, @answer, @criteria, @defaultSeverity, 1)`);
            created++;
            existingRefs.add(refLower);
        }
...
router.put('/api/templates/items/:itemId', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('range2To', sql.Int, range2To != null ? range2To : null)
            .input('range3From', sql.Int, range3From != null ? range3From : null)
            .query(`UPDATE SEC_InspectionTemplateItems SET ReferenceValue = @ref, Question = @question, Coefficient = @coeff, Quantity = @quantity, AnswerOptions = @answer, Criteria = @criteria, DefaultSeverity = @defaultSeverity, SubCategory = @subCategory, IsQuantitative = @isQuantitative, Range1From = @range1From, Range1To = @range1To, Range2From = @range2From, Range2To = @range2To, Range3From = @range3From WHERE Id = @id`);
        await pool.close();
        res.json({ success: true });
    } catch (error) {
...
router.delete('/api/templates/items/:itemId', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.Int, req.params.itemId)
            .query(`UPDATE SEC_InspectionTemplateItems SET IsActive = 0 WHERE Id = @id`);
        await pool.close();
        res.json({ success: true });
    } catch (error) {
...
router.delete('/api/templates/categories/:categoryId/items/all', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        const result = await pool.request()
            .input('categoryId', sql.Int, req.params.categoryId)
            .query(`UPDATE SEC_InspectionTemplateItems SET IsActive = 0 WHERE CategoryId = @categoryId; SELECT @@ROWCOUNT as deleted`);
        await pool.close();
...
// Legacy: Delete all items in section
router.delete('/api/templates/sections/:sectionId/items/all', async (req, res) => {
    try {
        const pool = await sql.connect(dbConfig);
        const result = await pool.request()
            .input('sectionId', sql.Int, req.params.sectionId)
            .query(`UPDATE SEC_InspectionTemplateItems SET IsActive = 0 WHERE SectionId = @sectionId; SELECT @@ROWCOUNT as deleted`);
        await pool.close();
        res.json({ success: true, data: { deleted: result.recordset[0].deleted } });
    } catch (error) {
        console.error('Error deleting items:', error);
...
router.get('/api/stores', async (req, res) => {
    try {
        ...
        const result = await pool.request().query(`
            ...
            FROM Stores s
            LEFT JOIN SEC_InspectionTemplates t ON s.TemplateId = t.Id
    ...
    try {
        ...
        for (const [key, value] of Object.entries(settings)) {
            await pool.request()
                ...
                .query(`
                    IF EXISTS (SELECT 1 FROM SEC_InspectionSettings WHERE SettingKey = @key)
                        UPDATE SEC_InspectionSettings SET SettingValue = @value, UpdatedAt = GETDATE() WHERE SettingKey = @key
                    ELSE
                        INSERT INTO SEC_InspectionSettings (SettingKey, SettingValue) VALUES (@key, @value)
                `);
        }
...
router.get('/api/next-document-number', async (req, res) => {
    try {
        ...
        // Get prefix from settings (default: GMRL-SEC)
        const prefixResult = await pool.request().query(`
            SELECT SettingValue FROM SEC_InspectionSettings WHERE SettingKey = 'DOCUMENT_PREFIX'
        `);
...
router.post('/api/inspections', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO SEC_Inspections (DocumentNumber, StoreId, StoreName, InspectionDate, Inspectors, AccompaniedBy, Status, CreatedBy, CreatedAt)
                OUTPUT INSERTED.Id
        ...
        if (!useTemplateId) {
            const defaultTemplate = await pool.request().query(`
                SELECT TOP 1 Id FROM SEC_InspectionTemplates WHERE IsDefault = 1 AND IsActive = 1
            `);
        ...
        if (useTemplateId) {
            ...
            for (const section of templateSections.recordset) {
                ...
                const sectionResult = await pool.request()
                    ...
                    .query(`
                        INSERT INTO SEC_InspectionSections (InspectionId, SectionName, SectionIcon, SectionOrder)
                        OUTPUT INSERTED.Id
                        VALUES (@inspectionId, @sectionName, @sectionIcon, @sectionOrder)
                    `);
                
                // Copy items for this section (includes items under categories)
                ...
                for (const item of templateItems.recordset) {
                    await pool.request()
                        ...
                        .input('quantity', sql.Int, item.Quantity || null)
                        .input('answerOptions', sql.NVarChar, item.AnswerOptions || 'Yes,No,NA')
                        ...
                        .query(`
                            INSERT INTO SEC_InspectionItems 
...
router.get('/api/inspections/:id', async (req, res) => {
    try {
        ...
        const inspection = await pool.request()
            .input('id', sql.Int, id)
            .query(`SELECT * FROM SEC_Inspections WHERE Id = @id`);
        
        const items = await pool.request()
            .input('id', sql.Int, id)
            .query(`SELECT * FROM SEC_InspectionItems WHERE InspectionId = @id ORDER BY SectionOrder, ItemOrder`);
        ...
        if (items && items.length > 0) {
            for (const item of items) {
                await pool.request()
                    ...
                    .query(`
                        IF EXISTS (SELECT 1 FROM SEC_InspectionItems WHERE InspectionId = @inspectionId AND SectionName = @sectionName AND ItemOrder = @itemOrder)
                            UPDATE SEC_InspectionItems SET Answer = @answer, Score = @score, Finding = @finding, Priority = @priority WHERE InspectionId = @inspectionId AND SectionName = @sectionName AND ItemOrder = @itemOrder
                        ELSE
                            INSERT INTO SEC_InspectionItems (InspectionId, SectionName, ItemOrder, Question, Answer, Score, Finding, Priority) VALUES (@inspectionId, @sectionName, @itemOrder, @question, @answer, @score, @finding, @priority)
                    `);
            }
        }
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionActionItems ai
                LEFT JOIN SEC_ActionItemVerification v ON v.ActionItemId = ai.Id
        ...
        for (const item of items) {
            if (item.id) {
                ...
                await pool.request()
                    ...
                    .query(`
                        UPDATE SEC_InspectionActionItems 
                ...
                await pool.request()
                    ...
                    .query(`
                        INSERT INTO SEC_InspectionActionItems (InspectionId, Finding, Action, Responsible, Deadline, Priority, Status, CreatedAt)
...
router.get('/api/audits/list', async (req, res) => {
    try {
        ...
        const result = await pool.request().query(`
            ...
            FROM SEC_Inspections i
            LEFT JOIN Users u ON i.CreatedBy = u.Id
            LEFT JOIN SEC_InspectionTemplates t ON t.IsDefault = 1 AND t.IsActive = 1
            LEFT JOIN SEC_InspectionSettings s ON s.SettingKey = 'PASSING_SCORE'
            ORDER BY i.CreatedAt DESC
        `);
        await pool.close();
    ...
    try {
        ...
        let sectionsResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionSections s
                WHERE s.InspectionId = @inspectionId
                ORDER BY s.SectionOrder
            `);
        
        // If no sections exist, populate from template
        let templateId = audit.TemplateId ? parseInt(audit.TemplateId) : null;
        
        // If no templateId on audit, try to find one
        if (!templateId) {
            const defaultTpl = await pool.request().query(`
                SELECT TOP 1 Id FROM SEC_InspectionTemplates WHERE IsActive = 1 ORDER BY IsDefault DESC, Id DESC
            `);
            templateId = defaultTpl.recordset[0]?.Id || null;
            // Save the templateId to the audit
            if (templateId) {
                await pool.request()
                    .input('templateId', sql.Int, templateId)
                    .input('auditId', sql.Int, auditId)
                    .query('UPDATE SEC_Inspections SET TemplateId = @templateId WHERE Id = @auditId');
            }
        }
...
router.delete('/api/audits/:auditId', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.Int, auditId)
            .query(`DELETE FROM SEC_FridgeReadings WHERE InspectionId = @id`);
        
        await pool.request()
            .input('id', sql.Int, auditId)
            .query(`DELETE FROM SEC_InspectionActionItems WHERE InspectionId = @id`);
        
        await pool.request()
            .input('id', sql.Int, auditId)
            .query(`DELETE FROM SEC_InspectionItems WHERE InspectionId = @id`);
        
        await pool.request()
            .input('id', sql.Int, auditId)
            .query(`DELETE FROM SEC_InspectionSections WHERE InspectionId = @id`);
        
        // Finally delete the audit itself
        const result = await pool.request()
            .input('id', sql.Int, auditId)
            .query(`DELETE FROM SEC_Inspections WHERE Id = @id; SELECT @@ROWCOUNT as deleted`);
    ...
    try {
        ...
        const auditResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_Inspections i
                LEFT JOIN SEC_InspectionTemplates t ON i.TemplateId = t.Id
                WHERE i.Id = @auditId
            `);
        ...
        const fridgeResult = await pool.request()
            .input('auditId', sql.Int, auditId)
            .query(`
                SELECT * FROM SEC_FridgeReadings 
                WHERE InspectionId = @auditId
                ORDER BY Id
            `);
        ...
        const picturesResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionPictures p
                INNER JOIN SEC_InspectionItems i ON p.ItemId = i.Id
        ...
        const galleryPicturesResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionGalleryLinks l
                INNER JOIN SEC_InspectionGallery g ON g.Id = l.GalleryPictureId
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .score-pass { color: #dc2626; font-weight: 600; }
        ...
        .chart-row-bar.bar-pass { background: linear-gradient(90deg, #dc2626 0%, #b91c1c 100%); }
        ...
        .chart-row-value.bar-pass { color: #dc2626; }
        ...
        .legend-color.pass { background: #dc2626; }
        ...
        .choice-yes { color: #dc2626; font-weight: 600; }
        ...
        .finding-cr { background: #fef2f2; border-left: 4px solid #dc2626; padding: 10px; margin-top: 8px; border-radius: 4px; font-size: 13px; color: #065f46; }
        ...
        .gallery-type.corrective { background: #fee2e2; color: #065f46; }
        ...
        .filter-btn.filter-yes.active { border-color: #dc2626; background: #fef2f2; color: #b91c1c; }
...
<body>
    ...
    <script>
        function goBack() {
            if (document.referrer && document.referrer.includes(window.location.hostname)) {
                ...
                window.location.href = '/security-inspection/reports';
            }
        }
    ...
    <div class="container">
                    ...
                    <tbody>
                        ${(section.items || []).map(item => {
                            ...
                            const goodPicsHtml = goodPics.length > 0 
                                ? goodPics.map(p => '<img src="' + p.dataUrl + '" alt="Good" title="Good Observation" style="max-width:50px;max-height:40px;border-radius:4px;cursor:pointer;border:2px solid #dc2626;object-fit:cover;" onclick="openLightbox(this.src)">').join('') 
                                : '-';
                            return `
                    ...
                    ${sectionFindings.map(f => {
                        ...
                        const picsHtml = itemPics.length > 0 
                            ? '<div class="finding-pictures"><strong>Photos:</strong><div class="pictures-wrapper">' + 
                              itemPics.map(p => '<img src="' + p.dataUrl + '" alt="' + (p.fileName || 'Photo') + '" title="' + (p.pictureType || 'Photo') + '" style="max-width:100px;max-height:75px;border-radius:4px;cursor:pointer;border:2px solid ' + (p.pictureType === 'Good' || p.pictureType === 'corrective' ? '#dc2626' : '#ef4444') + ';" onclick="openLightbox(this.src)">').join('') + 
                              '</div></div>' 
                            : '';
                        return `
        ...
        <div class="footer">
            Report generated on ${new Date(generatedAt).toLocaleString()} | Security Inspection System
        </div>
    </div>
    ...
    <script>
        ...
        async function openEmailModal(reportType) {
            try {
                ...
                const recipientsRes = await fetch('/security-inspection/api/audits/' + auditId + '/email-recipients?reportType=' + reportType);
                ...
                const previewRes = await fetch('/security-inspection/api/audits/' + auditId + '/email-preview?reportType=' + reportType);
                ...
                EmailModal.show({
                    ...
                    auditId: auditId,
                    sendUrl: '/security-inspection/api/audits/' + auditId + '/send-report-email',
...
router.put('/api/audits/response/:responseId', async (req, res) => {
    try {
        ...
        const currentResult = await pool.request()
            ...
            .query(`SELECT Answer, Score, Finding, Comment, CorrectedAction, Priority, Escalate, Department, Quantity, ActualQuantity, Deadline FROM SEC_InspectionItems WHERE Id = @id`);
        ...
        if (escalate !== undefined && finalDepartment) {
            try {
                ...
                if (finalEscalate && finalDepartment) {
                    ...
                    await deptEscalationService.escalateToDepepartment('SEC', responseId, finalDepartment, {
                        id: user.id,
                    ...
                    await deptEscalationService.removeEscalation('SEC', responseId);
                }
            } catch (escErr) {
...
router.post('/api/audits/pictures', oeAuditUpload.single('picture'), async (req, res) => {
    try {
        ...
        const filePath = '/uploads/security-inspection/' + req.file.filename;
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO SEC_InspectionPictures (ItemId, InspectionId, FileName, OriginalName, ContentType, PictureType, FilePath, FileSize, CreatedAt)
        ...
        await pool.request()
            .input('id', sql.Int, responseId)
            .query(`UPDATE SEC_InspectionItems SET HasPicture = 1 WHERE Id = @id`);
...
router.delete('/api/audits/pictures/:pictureId', async (req, res) => {
    try {
        ...
        const fileResult = await pool.request()
            ...
            .query(`SELECT FilePath FROM SEC_InspectionPictures WHERE Id = @id`);
        ...
        await pool.request()
            .input('id', sql.Int, pictureId)
            .query(`DELETE FROM SEC_InspectionPictures WHERE Id = @id`);
    ...
    try {
        ...
        for (const finding of findingsResult.recordset) {
            ...
            const existingCheck = await pool.request()
                ...
                .query(`
                    SELECT Id FROM SEC_InspectionActionItems 
                    WHERE InspectionId = @inspectionId 
            ...
            if (existingCheck.recordset.length === 0) {
                await pool.request()
                    ...
                    .query(`
                        INSERT INTO SEC_InspectionActionItems 
        ...
        if (actionItemsCreated > 0) {
            try {
                const settingsResult = await pool.request()
                    .query("SELECT SettingValue FROM SEC_EscalationSettings WHERE SettingKey = 'ActionPlanDeadlineDays'");
...
router.post('/api/audits/:auditId/fridge-readings', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('auditId', sql.Int, auditId)
            .query(`DELETE FROM SEC_FridgeReadings WHERE InspectionId = @auditId`);
        ...
        for (const reading of (goodReadings || [])) {
            await pool.request()
                ...
                .query(`
                    INSERT INTO SEC_FridgeReadings (InspectionId, DocumentNumber, ReadingType, UnitTemp, DisplayTemp, ProbeTemp, SectionName, CreatedAt)
        ...
        for (const reading of (badReadings || [])) {
            await pool.request()
                ...
                .input('sectionName', sql.NVarChar, reading.sectionName || null)
                .query(`
                    INSERT INTO SEC_FridgeReadings (InspectionId, DocumentNumber, ReadingType, UnitTemp, DisplayTemp, ProbeTemp, Issue, SectionName, CreatedAt)
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT 
                    ...
                    u.DisplayName as VerifiedByName
                FROM SEC_InspectionActionItems a
                INNER JOIN SEC_Inspections i ON a.InspectionId = i.Id
                LEFT JOIN SEC_ActionItemVerification v ON v.ActionItemId = a.Id
                LEFT JOIN Users u ON v.VerifiedBy = u.Id
...
router.post('/api/action-plan/save', async (req, res) => {
    try {
        ...
        const inspResult = await pool.request()
            .input('documentNumber', sql.NVarChar, documentNumber)
            .query(`SELECT Id FROM SEC_Inspections WHERE DocumentNumber = @documentNumber`);
        ...
        const completionCheck = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionActionItems 
                WHERE InspectionId = @inspectionId
            `);
        ...
        if (TotalItems > 0 && TotalItems === CompletedItems) {
            ...
            await pool.request()
                .input('inspectionId', sql.Int, inspectionId)
                .query(`
                    UPDATE SEC_ActionPlanEscalations 
...
router.post('/api/action-plan/upload-picture', oeAuditUpload.single('picture'), async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            url: `/uploads/security-inspection/${req.file.filename}`,
...
router.get('/api/schemas', async (req, res) => {
    try {
        ...
        const result = await pool.request().query(`
            ...
            FROM SEC_InspectionTemplates t
            LEFT JOIN SEC_InspectionSettings s ON s.SettingKey = 'PASSING_SCORE_' + CAST(t.Id AS VARCHAR)
            WHERE t.IsActive = 1
            ORDER BY t.TemplateName
        `);
        ...
        for (const schema of result.recordset) {
            const sectionsResult = await pool.request()
                ...
                .query(`
                    ...
                    FROM SEC_InspectionTemplateSections ts
...
router.get('/api/schema-colors/:schemaId', async (req, res) => {
    try {
        ...
        res.json({ success: true, colors: { passColor: '#dc2626', failColor: '#ef4444', headerColor: '#1e3a5f', accentColor: '#dc2626' } });
    }
});
...
router.get('/api/schema-checklist-info/:schemaId', async (req, res) => {
    try {
        ...
        const info = {
            ...
            revisionDate: '',
            edition: '',
            reportTitle: 'Security Inspection Report',
            documentPrefix: 'GMRL-SEC'
        };
        ...
        res.json({ success: true, info: { creationDate: '', revisionDate: '', edition: '', reportTitle: 'Security Inspection Report', documentPrefix: 'GMRL-SEC' } });
    }
});
...
router.get('/api/action-items', async (req, res) => {
    try {
        ...
        let query = `
            ...
            FROM SEC_InspectionActionItems ai
            INNER JOIN SEC_Inspections i ON ai.InspectionId = i.Id
            WHERE 1=1
        `;
...
router.post('/api/schedules', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO SEC_InspectionSchedule (InspectorId, StoreId, TemplateId, ScheduledDate, Notes, CreatedBy)
                OUTPUT INSERTED.Id
...
router.delete('/api/schedules/:id', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.Int, id)
            .query('DELETE FROM SEC_InspectionSchedule WHERE Id = @id');
    ...
    try {
        ...
        if (status === 'pending') {
            ...
            statusFilter = `AND EXISTS (
                SELECT 1 FROM SEC_InspectionActionItems ai 
                WHERE ai.InspectionId = i.Id
                AND NOT EXISTS (SELECT 1 FROM SEC_ActionItemVerification v WHERE v.ActionItemId = ai.Id)
            )`;
            ...
            statusFilter = `AND NOT EXISTS (
                ...
                WHERE ai.InspectionId = i.Id
                AND NOT EXISTS (SELECT 1 FROM SEC_ActionItemVerification v WHERE v.ActionItemId = ai.Id)
            ) AND EXISTS (SELECT 1 FROM SEC_ActionItemVerification v WHERE v.InspectionId = i.Id)`;
        }
        ...
        const result = await pool.request()
            .query(`
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM SEC_InspectionActionItems ai WHERE ai.InspectionId = i.Id) as TotalActionItems,
                    (SELECT COUNT(*) FROM SEC_InspectionActionItems ai WHERE ai.InspectionId = i.Id AND ai.Status IN ('Completed', 'In Progress', 'Deferred')) as RespondedItems,
                    (SELECT COUNT(*) FROM SEC_ActionItemVerification v WHERE v.InspectionId = i.Id) as VerifiedItems,
                    (SELECT COUNT(*) FROM SEC_ActionItemVerification v WHERE v.InspectionId = i.Id AND v.VerificationStatus = 'Verified Complete') as VerifiedComplete,
                    (SELECT COUNT(*) FROM SEC_ActionItemVerification v WHERE v.InspectionId = i.Id AND v.VerificationStatus = 'Verified Not Complete') as VerifiedNotComplete
                FROM SEC_Inspections i
                WHERE i.Status = 'Completed'
                  AND EXISTS (SELECT 1 FROM SEC_InspectionActionItems ai WHERE ai.InspectionId = i.Id)
                  ${statusFilter}
                ORDER BY i.Id DESC
            `);
    ...
    try {
        ...
        const inspectionResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_Inspections WHERE Id = @inspectionId
            `);
        ...
        const actionItemsResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionActionItems ai
                LEFT JOIN SEC_InspectionItems ii ON ii.InspectionId = ai.InspectionId AND ii.ReferenceValue = ai.ReferenceValue
                LEFT JOIN SEC_ActionItemVerification v ON v.ActionItemId = ai.Id
                LEFT JOIN Users u ON v.VerifiedBy = u.Id
                LEFT JOIN DepartmentEscalations de ON de.InspectionId = ai.InspectionId AND de.ReferenceValue = ai.ReferenceValue AND de.Module = 'SEC'
                WHERE ai.InspectionId = @inspectionId
...
router.post('/api/verification/submit', async (req, res) => {
    try {
        ...
        const existingResult = await pool.request()
            ...
            .query('SELECT Id FROM SEC_ActionItemVerification WHERE ActionItemId = @actionItemId');
        ...
        if (existingResult.recordset.length > 0) {
            ...
            await pool.request()
                ...
                .query(`
                    UPDATE SEC_ActionItemVerification 
            ...
            await pool.request()
                ...
                .input('verificationPictureUrl', sql.NVarChar, verificationPictureUrl || null)
                .query(`
                    INSERT INTO SEC_ActionItemVerification 
...
router.get('/api/verification/stats', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            .query(`
                SELECT 
                    (SELECT COUNT(DISTINCT i.Id) FROM SEC_Inspections i
                     WHERE i.Status = 'Completed'
                     AND EXISTS (
                         SELECT 1 FROM SEC_InspectionActionItems ai 
                         WHERE ai.InspectionId = i.Id
                    ...
                    (SELECT COUNT(DISTINCT i.Id) FROM SEC_Inspections i
                     WHERE EXISTS (SELECT 1 FROM SEC_ActionItemVerification v WHERE v.InspectionId = i.Id)) as Verified,
                    (SELECT COUNT(*) FROM SEC_ActionItemVerification WHERE VerificationStatus = 'Verified Complete') as VerifiedComplete,
                    (SELECT COUNT(*) FROM SEC_ActionItemVerification WHERE VerificationStatus = 'Verified Not Complete') as VerifiedNotComplete
            `);
    ...
    try {
        ...
        const itemResult = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionActionItems ai
                JOIN SEC_Inspections i ON ai.InspectionId = i.Id
                LEFT JOIN SEC_InspectionSections s ON ai.SectionId = s.Id
                LEFT JOIN SEC_ActionItemVerification v ON ai.Id = v.ActionItemId
                LEFT JOIN Users u ON v.VerifiedBy = u.Id
        ...
        // Get email template
        const templateResult = await pool.request()
            .input('templateKey', sql.NVarChar, 'SEC_VERIFICATION_SUBMITTED')
        ...
        const variables = {
            ...
            verificationUrl: `https://oeapp.gmrlapps.com/security-inspection/implementation-verification/${item.InspectionId}`,
...
router.post('/api/verification/send-email', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`
                INSERT INTO SEC_VerificationEmailLog (ActionItemId, ToEmail, Subject, SentBy, SentAt)
...
function buildVerificationSummaryEmail(inspection, actionItems, stats) {
    const itemsHtml = actionItems.map(item => {
        ...
        if (item.VerificationStatus === 'Verified Complete') {
            statusBadge = '<span style="background:#fee2e2;color:#b91c1c;padding:4px 10px;border-radius:12px;font-size:12px;">✅ Complete</span>';
    ...
    return '<!DOCTYPE html>' +
        ...
        '<div style="flex:1;background:#fee2e2;padding:15px;border-radius:10px;"><div style="font-size:28px;font-weight:700;color:#b91c1c;">' + stats.verifiedComplete + '</div><div style="font-size:12px;color:#b91c1c;">Verified Complete</div></div>' +
        ...
        '</div>' +
        '<h3 style="margin-top:30px;border-bottom:2px solid #dc2626;padding-bottom:10px;">📋 Action Items</h3>' +
        ...
        '<a href="https://oeapp.gmrlapps.com/security-inspection/implementation-verification/' + inspection.Id + '" style="display:inline-block;padding:14px 30px;background:#dc2626;color:white;text-decoration:none;border-radius:8px;font-weight:600;">🔍 View Full Details</a>' +
        '</div>' +
        ...
        '<p>This is an automated message from the Security Department Application.</p>' +
...
router.post('/api/verification/send-email-full', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('sentBy', sql.Int, req.session?.user?.id || null)
            .query(`
                INSERT INTO SEC_VerificationEmailLog (InspectionId, ToEmail, Subject, SentBy, SentAt)
...
router.get('/api/audits/:auditId/email-preview', async (req, res) => {
    try {
        ...
        const reportUrl = reportType === 'action-plan' 
            ? `${appUrl}/security-inspection/api/audits/reports/SEC_ActionPlan_${audit.DocumentNumber}_${new Date().toISOString().split('T')[0]}.html`
            : `${appUrl}/security-inspection/api/audits/reports/SEC_Report_${audit.DocumentNumber}_${new Date().toISOString().split('T')[0]}.html`;
        ...
        const emailContent = await emailTemplateBuilder.buildEmailFromDB('SEC', reportType, auditData, reportUrl, findingsStats);
        ...
        for (const brand of brandsResult.recordset) {
            ...
            const storeAuditsResult = await pool.request()
                ...
                .query(`
                    ...
                    FROM Stores s
                    LEFT JOIN SEC_Inspections i ON s.Id = i.StoreId AND i.Status = 'Completed'
...
router.get('/api/department-escalations', async (req, res) => {
    try {
        ...
        const isAdmin = req.currentUser?.hasRole?.('System Administrator') || 
                        req.currentUser?.hasRole?.('Head of Security Department');
        const userDept = req.currentUser?.department;
        
        let query = `SELECT * FROM DepartmentEscalations WHERE Module = 'SEC'`;
...
router.get('/api/department-escalations/stats', async (req, res) => {
    try {
        ...
        let whereClause = `WHERE Module = 'SEC'`;
        if (!isAdmin && userDept) {
...
const galleryUploadDir = path.join(__dirname, '..', '..', 'uploads', 'security-inspection', 'gallery');
...
router.post('/api/gallery/:auditId/upload', galleryUpload.single('picture'), async (req, res) => {
    try {
        ...
        const result = await pool.request()
            .input('auditId', sql.Int, auditId)
            .input('fileName', sql.NVarChar, req.file.filename)
            .input('filePath', sql.NVarChar, '/uploads/security-inspection/gallery/' + req.file.filename)
            .input('originalName', sql.NVarChar, req.file.originalname)
            .input('fileSize', sql.Int, stats.size)
            .input('uploadedBy', sql.NVarChar, user ? user.displayName : 'Unknown')
            .query(`
                INSERT INTO SEC_InspectionGallery (AuditId, FileName, FilePath, OriginalName, FileSize, UploadedBy)
    ...
    try {
        ...
        for (const file of req.files) {
            ...
            const result = await pool.request()
                ...
                .input('filePath', sql.NVarChar, '/uploads/security-inspection/gallery/' + file.filename)
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM SEC_InspectionGalleryLinks WHERE GalleryPictureId = g.Id) as AssignmentCount
                FROM SEC_InspectionGallery g
                WHERE g.AuditId = @auditId
...
router.delete('/api/gallery/picture/:pictureId', async (req, res) => {
    try {
        ...
        const fileResult = await pool.request()
            .input('id', sql.Int, pictureId)
            .query('SELECT FilePath FROM SEC_InspectionGallery WHERE Id = @id');
        ...
        await pool.request()
            .input('id', sql.Int, pictureId)
            .query('DELETE FROM SEC_InspectionGallery WHERE Id = @id');
...
router.post('/api/gallery/assign', async (req, res) => {
    try {
        ...
        const existing = await pool.request()
            ...
            .input('pictureType', sql.NVarChar, pictureType)
            .query(`
                SELECT Id FROM SEC_InspectionGalleryLinks 
        ...
        const result = await pool.request()
            ...
            .input('pictureType', sql.NVarChar, pictureType)
            .query(`
                INSERT INTO SEC_InspectionGalleryLinks (GalleryPictureId, ResponseId, PictureType)
                OUTPUT INSERTED.Id
...
router.delete('/api/gallery/unassign/:linkId', async (req, res) => {
    try {
        ...
        const linkResult = await pool.request()
            ...
            .query('SELECT ResponseId FROM SEC_InspectionGalleryLinks WHERE Id = @id');
        ...
        await pool.request()
            .input('id', sql.Int, linkId)
            .query('DELETE FROM SEC_InspectionGalleryLinks WHERE Id = @id');
        ...
        const pictureCheck = await pool.request()
            ...
            .query(`
                SELECT 
                    (SELECT COUNT(*) FROM SEC_InspectionPictures WHERE ResponseId = @responseId) +
                    (SELECT COUNT(*) FROM SEC_InspectionGalleryLinks WHERE ResponseId = @responseId) as TotalPictures
            `);
        ...
        if (pictureCheck.recordset[0].TotalPictures === 0) {
            await pool.request()
                .input('responseId', sql.Int, responseId)
                .query('UPDATE SEC_InspectionItems SET HasPicture = 0 WHERE Id = @responseId');
        }
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                FROM SEC_InspectionGalleryLinks l
                INNER JOIN SEC_InspectionItems i ON i.Id = l.ResponseId
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                WHERE g.AuditId = @auditId
                AND NOT EXISTS (
                    SELECT 1 FROM SEC_InspectionGalleryLinks l WHERE l.GalleryPictureId = g.Id
                )
                ORDER BY g.UploadedAt DESC
            `);
...
router.delete('/api/gallery/:auditId/unassigned', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('auditId', sql.Int, auditId)
            .query(`
                DELETE FROM SEC_InspectionGallery 
                WHERE AuditId = @auditId
                AND NOT EXISTS (
                    SELECT 1 FROM SEC_InspectionGalleryLinks l WHERE l.GalleryPictureId = SEC_InspectionGallery.Id
                )
            `);
...
// ==========================================
// Maintenance routes not included
// Maintenance routes not included
