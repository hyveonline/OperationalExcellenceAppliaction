            ...
            <head>
                ...
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .items-table th { background: #f8f9fa; padding: 12px; text-align: left; font-size: 13px; font-weight: 600; }
                    .items-table td { padding: 10px 12px; border-bottom: 1px solid #eee; }
                    .items-table input { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; }
            ...
            <body>
                        ...
                        <form id="editForm">
                            ...
                            <div class="form-section">
                                ...
                                <div class="form-row">
                                    <div class="form-group">
                                        <label>Date *</label>
                                        <input type="date" id="logDate" value="${logDate}" required>
                                    </div>
                                    <div class="form-group">
                                        <label>Premises *</label>
                                        <select id="premises" required>
                                    ...
                                    <div class="form-group">
                                        <label>Filled By *</label>
                                        <input type="text" id="filledBy" value="${log.FilledBy}" required>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="form-section">
                                <div class="section-title">📦 Delivery Items <button type="button" class="btn btn-outline" onclick="addItem()" style="margin-left:auto;padding:8px 16px;font-size:13px;">+ Add</button></div>
                                <table class="items-table">
                                    <thead><tr><th>#</th><th>Employee Name</th><th>Received From</th><th>Time</th><th>Notes</th><th></th></tr></thead>
                                    <tbody id="itemsBody">${itemRows}</tbody>
                                </table>
                            </div>
                ...
                <script>
                    ...
                    function addItem() {
                        itemCount++;
                        const row = document.createElement('tr');
                        row.innerHTML = '<td>'+itemCount+'</td><td><input type="text" name="items['+(itemCount-1)+'][employeeName]" required></td><td><input type="text" name="items['+(itemCount-1)+'][receivedFrom]"></td><td><input type="time" name="items['+(itemCount-1)+'][time]" required></td><td><input type="text" name="items['+(itemCount-1)+'][notes]"></td><td><button type="button" class="btn btn-danger" onclick="removeItem(this)">✕</button></td>';
                        document.getElementById('itemsBody').appendChild(row);
                    }
                    function removeItem(btn) { btn.closest('tr').remove(); }
                    function showAlert(msg, type) { const a = document.getElementById('alertBox'); a.textContent = msg; a.style.display = 'block'; }
                    
                    document.getElementById('editForm').addEventListener('submit', async (e) => {
                        e.preventDefault();
                        const items = [];
                        document.querySelectorAll('#itemsBody tr').forEach((row, idx) => {
                            const emp = row.querySelector('input[name="items['+idx+'][employeeName]"]');
                            const rec = row.querySelector('input[name="items['+idx+'][receivedFrom]"]');
                            const tim = row.querySelector('input[name="items['+idx+'][time]"]');
                            const not = row.querySelector('input[name="items['+idx+'][notes]"]');
                            if (emp && emp.value && tim && tim.value) {
                                items.push({ employeeName: emp.value, receivedFrom: rec?.value||'', time: tim.value, notes: not?.value||'' });
                            }
                        });
                        if (items.length === 0) { showAlert('Add at least one item'); return; }
                        try {
                            const res = await fetch('/security-services/delivery-log/update', {
                                method: 'POST', headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ logId: document.getElementById('logId').value, logDate: document.getElementById('logDate').value, premises: document.getElementById('premises').value, filledBy: document.getElementById('filledBy').value, items })
                            });
                            const result = await res.json();
                            if (result.success) window.location.href = '/security-services/delivery-log/' + document.getElementById('logId').value;
                            else showAlert(result.error || 'Failed');
...
router.post('/update', async (req, res) => {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            const timeValue = item.time.length === 5 ? item.time + ':00' : item.time;
            await pool.request()
                .input('deliveryLogId', sql.Int, logId)
                .input('employeeName', sql.NVarChar, item.employeeName)
                .input('receivedFrom', sql.NVarChar, item.receivedFrom || '')
                ...
                .input('notes', sql.NVarChar, item.notes || '')
                .input('itemOrder', sql.Int, i + 1)
                .query(`INSERT INTO Security_DeliveryLogItems (DeliveryLogId, EmployeeName, ReceivedFrom, DeliveryTime, Notes, ItemOrder) VALUES (@deliveryLogId, @employeeName, @receivedFrom, CAST(@deliveryTime AS TIME), @notes, @itemOrder)`);
        }
