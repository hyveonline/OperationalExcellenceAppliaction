...
router.get('/:id/edit', async (req, res) => {
    try {
        ...
        const form = formResult.recordset[0];
        const formDate = new Date(form.FormDate).toISOString().split('T')[0];
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="entrance" required>
                                            <option value="Main Entrance" \${form.Entrance === 'Main Entrance' ? 'selected' : ''}>Main Entrance</option>
                                            <option value="Side Entrance" \${form.Entrance === 'Side Entrance' ? 'selected' : ''}>Side Entrance</option>
                                            <option value="Back Entrance" \${form.Entrance === 'Back Entrance' ? 'selected' : ''}>Back Entrance</option>
                                            <option value="Parking Entrance" \${form.Entrance === 'Parking Entrance' ? 'selected' : ''}>Parking Entrance</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Location *</label>
                ...
                <script>
                    ...
                    function addEntry() {
                        ...
                        row.innerHTML = '<td>' + entryCount + '</td>' +
                            ...
                            '<td><input type="time" name="entries[' + (entryCount-1) + '][timeIn]" required></td>' +
                            '<td><input type="time" name="entries[' + (entryCount-1) + '][timeOut]"></td>' +
                            '<td><input type="text" name="entries[' + (entryCount-1) + '][guardName]"></td>' +
                            '<td><button type="button" class="btn btn-danger" onclick="removeEntry(this)">✕</button></td>';
                        tbody.appendChild(row);
                    }
                    ...
                    document.getElementById('editForm').addEventListener('submit', async (e) => {
                        ...
                        document.querySelectorAll('#entriesBody tr').forEach((row, index) => {
                            ...
                            const contractor = row.querySelector('input[name="entries[' + index + '][contractor]"]')?.value;
...
router.post('/update', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(\`
                UPDATE Security_EntranceForms 
                SET FormDate = @formDate, Entrance = @entrance, Location = @location, UpdatedAt = GETDATE(), UpdatedBy = @updatedBy
                WHERE Id = @id
            \`);
        
        await pool.request()
            .input('formId', sql.Int, formId)
            .query('DELETE FROM Security_EntranceEntries WHERE EntranceFormId = @formId');
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            await pool.request()
                ...
                .query(\`
                    INSERT INTO Security_EntranceEntries (EntranceFormId, FullName, Contractor, TimeIn, TimeOut, GuardName, EntryOrder)
                    VALUES (@formId, @fullName, @contractor, CAST(@timeIn AS TIME), \${timeOut ? 'CAST(@timeOut AS TIME)' : 'NULL'}, @guardName, @entryOrder)
                \`);
        }
