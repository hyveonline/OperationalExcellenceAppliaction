/**
 * Maintenance Integration Routes for OHS Inspection
 * Proxy routes to connect OHS Inspection with Maintenance WR system
 */
...
router.post('/create-wr', async (req, res) => {
    try {
        const { locationCode, priority, title, description, sourceApp, sourceType, sourceId, sourceRef } = req.body;
        
        // Validation
        if (!locationCode || !title || !description) {
            return res.status(400).json({ 
                success: false, 
                message: 'locationCode, title, and description are required' 
            });
        }
        
        // Get user info from session
        const requestedBy = req.session?.user?.displayName || req.session?.user?.name || 'OHS System';
        const requestedByEmail = req.session?.user?.email || 'ohs-system@gmrlgroup.com';
        ...
        const result = await maintenanceService.createWorkRequest({
            ...
            requestedBy,
            requestedByEmail,
            sourceApp: sourceApp || 'OHS_INSPECTION',
            sourceType: sourceType || 'Finding',
            sourceId,
            sourceRef
        });
        
        // If successful, update the OHS database to track the WR link
        if (result.success && result.data?.wrNumber && sourceId) {
            try {
                const pool = await sql.connect(dbConfig);
                // Try to update OHS_InspectionItems - columns may or may not exist
                await pool.request()
                ...
                console.error('Error updating OHS database with WR link:', dbError);
                // Don't fail the whole request for this - columns may not exist yet
            }
        }
 ...
 * Link an existing Work Request to an OHS finding
 */
 ...
 * Save maintenance WR link to OHS database directly (used by action-plan.html)
 */
